...
export default function Sidebar({ isCollapisible, setCollapisible }) {
  ...
  const SmallSizebar = () => {
    return (
      <div
        className={`${
          isCollapisible ? "w-20 block" : "hidden"
  ...
  return (
    <>
      {isCollapisible ? (
        ...
        <div
          className={`${
            isCollapisible ? "hidden" : ""
          } w-60 transition ease-in duration-250 fixed left-0 
    z-30 pt-4 pb-8 flex flex-col justify-between h-screen pl-2 pr-4 border-gray border-r-[1px]`}
        >
          <div>
            {!isCollapisible ? (
            ...
            <div className="">
              {navigationItem.map(({ id, subitem, description }) => {
                return (
                  <div
                    className="flex text-gray-400 flex-col gap-2 my-4"
                    key={id}
                  >
                    <p
                      className={`${
